...
<t:plantilla>
    <jsp:body>
        <h4>Seleccione la edición de curso a comentar</h4>
        <hr />
        <br />
	              ...
	              <tbody>
					<c:if test="${not empty usuario.getInscripEdiciones()}">
						<c:forEach items="${usuario.getInscripEdiciones()}" var="ed" >
		               		<tr>
				                <td><a class="link" href="ListarComentariosEdicionCurso?nombre=${fn:escapeXml(ed.getEdicion())}">${ed.getEdicion()}</a></td>
					...
					<c:if test="${empty usuario.getInscripEdiciones()}">
			                <tr>
			                  <td>No tiene inscripciones en las que pueda comentar...</td>
			                  <td></td>
			                  <td></td>
			                </tr>
            		</c:if>
